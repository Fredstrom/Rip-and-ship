...
button = {'size': (10, 2), 'font': ('Roboto Mono', 10, 'bold'), 'button_color': ("white", "#000000")}
...
def frontpage_window():
    layout = [
        [sg.Text('Rip and Ship', size=(30,1), justification='c',background_color="#54C5C9",
                       text_color='black', font=('Helvetica', 30, 'bold'))],
...
def inventory_window():
    layout = [
        [sg.Text('Inventory', size=(30, 1), justification='c', background_color="#54C5C9",
                  text_color='black', font=('Arial', 30, 'bold'))],
        [sg.Button('Exit')]
    ]
    return sg.Window('Inventory', layout, finalize=True,
                      background_color="#54C5C9", size=(500, 400), resizable=True, element_justification='center')
...
def main():
    ...
    while True:
        ...
        if window == window4 and (event in (sg.WIN_CLOSED, 'Exit')):
            window4.close()
            window4 = None
            window1.un_hide()
